 ...
 * @version 1.0
 */
public class RoundRobinScheduler implements Planificari{
    
/**
 * Aceasta este clasa planificarii RoundRobin.
 */
    ProblemData result;
    HomeworkWriter fout;
 ...
 */
    public RoundRobinScheduler(ProblemData result, HomeworkWriter fout) {
        this.result = result;
        this.fout = fout;
    }
 ...
 * incat diferenta in modul dintre oricare 2 procese, la orice moment de timp, 
 * sa fie mai mica sau egala cu 1. Metoda implementata de mine parcurge 
 * procesele pe rand. Declar un vector de dimensiunea cache-ului si il folosesc 
 * doar in cazul bonusului. Parcurg numerele pe rand, imi pun in variabile 
 * scurte numele/valorile unora dintre elemente, verific daca sunt in cazul 
 * bonusului si fac modificarile aferente, iar apoi pentru a evita scrierea a 7 
 * if-uri imi creez o instanta in functie de numele procesului (am aflat clasa, 
 * apoi am apelat constructorul si am obtinut instanta) pe care o adaug in 
 * vector. La final apelez metoda de afisare a tuturor actiunilor realizate.
 ...
 */
    ...
    public void generare(int verif_bonus) throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        ...
        for (int i = 0; i < result.getNumbersToBeProcessed().length; i++){
            int n = i % result.getProcesses().length;
